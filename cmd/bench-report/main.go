...
func main() {
	...
	if err := run(*bench, *benchtime, *count, *out, *resultsDir, *packages, *compare, *noRun, *openReport); err != nil {
		fmt.Fprintf(os.Stderr, "bench-report: %v\n", err)
		os.Exit(1)
	}
}

func run(bench, benchtime string, count int, out, resultsDir, packages string, compare int, noRun, openReport bool) error {
	...
	if !noRun {
		...
		for _, pkg := range pkgList {
			...
			if err := saveJSON(filepath.Join(resultsDir, fname), rec); err != nil {
